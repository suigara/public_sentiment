...
class CWebApplication extends CApplication
{
	...
	private $_controller;
	private $_theme;
	private $_perfreporter;
	private $_route;
	//edit by hardyzhu  无法解析url 的时候就不要上报，否则很多乱数据
	private $_action_exists=true;
	...
	public function processRequest()
	{
		...
		{
			$this->_route=$this->catchAllRequest[0];
			foreach(array_splice($this->catchAllRequest,1) as $name=>$value)
				$_GET[$name]=$value;
		}
		else
			$this->_route=$this->getUrlManager()->parseUrl($this->getRequest());
		$this->runController($this->_route);
	}
	...
	protected function init()
	{
		...
		$this->getRequest();
		$this->attachEventHandler('onBeginRequest',array($this,'processRequestBegin'));
		$this->attachEventHandler('onEndRequest',array($this,'processRequestEnd'));
	}
	
	public function processRequestBegin($event)
	{
		if(isset(Mod::app()->params->enableCgiPerform) && Mod::app()->params->enableCgiPerform == true){
			$_perf = new CPhpPerfReporter();
			$_perf->beginPerfReport(Mod::app()->id,'', 80, true );
			if(isset($_SERVER['IP_LOCAL'])){
				$_perf->setLocalIp($_SERVER['IP_LOCAL']);
			}
			else{   
				defined('IP_LOCAL') or define('IP_LOCAL', $_perf->genLocalIp());
				$_perf->setLocalIp(IP_LOCAL);
			}
			$this->_perfreporter  = $_perf ;
		}
	}

	public function processRequestEnd($event)
	{
		if(isset(Mod::app()->params->enableCgiPerform) 
			&& Mod::app()->params->enableCgiPerform == true
			&& isset($this->_controller) && $this->_action_exists){
			if(($route=trim($this->_route,'/'))===''){
				$urlca[0] = $this->defaultController;
				$urlca[1] = 'index';
			}
			else $urlca = explode("/",$route);
			$this->_perfreporter->setClassName($urlca[0]);
			$this->_perfreporter->setFunctionName($urlca[1]);
			$status = isset($event->params['status']) ? $event->params['status'] : 0 ;
			$this->_perfreporter->endPerfReport($status);
			unset($this->_perfreporter);
		}
	}
