...
class CDbAuthManager extends CAuthManager
{
	...
	public function addItemChild($itemName,$childName)
	{
		if($itemName===$childName)
			throw new CException(Mod::t('yii','Cannot add "{name}" as a child of itself.',
					array('{name}'=>$itemName)));
		...
		if(count($rows)==2)
		{
			...
			if($this->detectLoop($itemName,$childName))
				throw new CException(Mod::t('yii','Cannot add "{child}" as a child of "{name}". A loop has been detected.',
			...
			throw new CException(Mod::t('yii','Either "{parent}" or "{child}" does not exist.',array('{child}'=>$childName,'{parent}'=>$itemName)));
	}
	...
	public function assign($itemName,$userId,$bizRule=null,$data=null)
	{
		if($this->usingSqlite() && $this->getAuthItem($itemName)===null)
			throw new CException(Mod::t('yii','The item "{name}" does not exist.',array('{name}'=>$itemName)));
	...
	protected function getDbConnection()
	{
		if($this->db!==null)
			return $this->db;
		else if(($this->db=Mod::app()->getComponent($this->connectionID)) instanceof CDbConnection)
			return $this->db;
		else
			throw new CException(Mod::t('yii','CDbAuthManager.connectionID "{id}" is invalid. Please make sure it refers to the ID of a CDbConnection application component.',
