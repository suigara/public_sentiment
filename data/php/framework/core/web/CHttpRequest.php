 ...
 * @property string $hostInfo Schema and hostname part (with port number if needed) of the request URL (e.g. http://www.qq.com).
...
class CHttpRequest extends CApplicationComponent
{
	...
	public function disEncoding($string){
		$encoding = "UTF-8";
		if( mb_detect_encoding( $string, 'UTF-8,GBK') !== 'UTF-8'){
			$encoding = 'GBK';
		}
		return $encoding;
	}
	
	protected function xss_handle(&$v,$k){
		$e = $this->disEncoding($v);
		Mod::app()->charset = $e;
		if($v != strip_tags($v)){
			$obj = new CHtmlPurifier();
			$v = $obj->purify(htmlspecialchars($v, ENT_QUOTES,Mod::app()->charset));
		}
		else
			$v = htmlspecialchars($v, ENT_QUOTES,Mod::app()->charset);
	}
	...
	{
		if(isset(Mod::app()->params->enableUriXss) && Mod::app()->params->enableUriXss == true){
			if(isset($_GET) && !empty($_GET)){
				array_walk_recursive($_GET,array('CHttpRequest','xss_handle'));
			}
			if(isset($_POST) && !empty($_POST)){
				array_walk_recursive($_POST,array('CHttpRequest','xss_handle'));
			}
			if(isset($_REQUEST) && !empty($_REQUEST)){
				array_walk_recursive($_REQUEST,array('CHttpRequest','xss_handle'));
			}
			if(isset($_COOKIE) && !empty($_COOKIE)){
				array_walk_recursive($_COOKIE,array('CHttpRequest','xss_handle'));
			}
		}
		// normalize request
	 ...
	 * @return string schema and hostname part (with port number if needed) of the request URL (e.g. http://www.qq.om)
	 * @see setHostInfo
	 */
	...
	public function validateCsrfToken($event)
	{
		...
		{
			$reurl = $this->getUrlReferrer();
			foreach($this->validDomain as $k=>$v){
				if( strpos($reurl, $v) !== false){
					return ;
				}
			}
