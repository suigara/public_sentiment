...
abstract class PBMessage
{
    ...
    private function _ParseFromArray($length=99999999)
    {
        ...
        while ($this->reader->get_pointer() - $_begin < $length)
        {
            ...
            if (is_array($this->values[$messtypes['field']]))
            {
                ...
                /*
                if($messtypes['wired'] == 0) {
                    var_dump($this->values[$messtypes['field']][$index]->value);
                }
                */
            }
            else
            {
                ...
                /*
                if($messtypes['wired'] == 0) {
                    var_dump($this->values[$messtypes['field']]->value);
                }
                */
            }
    ...
    {
        if(method_exists($this, $name.'_size'))
        {
            return new PBRepeatedObject($this, $name);
        }else {
    ...
    public function __set($name,$value)
    {
        if(method_exists($this, $name.'_size'))
        {
            throw new Exception('Property "'.get_class($this).'.'.$name.'" Cannot be set directly.'); 
        }else {
            $setter='set_'.$name;
...
class PBRepeatedObject implements ArrayAccess ,Countable
{
    public $message;
    public $name ;
    ...
    public function offsetSet($offset, $value) {
        ...
        //echo "\nPBRepeatedObject.offsetSet:".$fn." ".$offset."  ".$value;
        $this->message->$fn($offset, $value);
    }
    public function offsetExists($offset) {
        $fn = $this->name;
        //echo "\nPBRepeatedObject.offsetExists:".$fn." ".$offset;
        $v = $this->message->$fn($offset);
        return isset($v);
    }
    public function offsetUnset($offset) {
        //echo "\nPBRepeatedObject.offsetUnSet:".$offset;
    }
    public function offsetGet($offset) {
        $fn = $this->name;
        //echo "\nPBRepeatedObject.offsetGet:".$fn." ".$offset;
        return $this->message->$fn($offset);
    }
    
    public function count()
    {
        $fn = $this->name."_size";
        return $this->message->$fn();
    }
