...
class PasClient extends CBehavior
{
	var $ticket;
	var $project;
	var $group;
	...
	function verifyTicket($project, $group, $type)
	{
        if(0){
            ...
            if($user){
                ...
                if(in_array($user, array('dolfeng','wilsonwsong','rusherding','vfreegu','pechspeng','riccohuang')))
                {
	    ...
	    Mod::log("passport:".($time_e-$time_s).'s', CLogger::LEVEL_TRACE, 'application.component.cwebpassuser');
		$ret = $array_recv[1];
		switch ($ret)
		{
			case '0':
