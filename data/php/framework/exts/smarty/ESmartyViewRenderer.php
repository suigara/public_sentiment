 ...
 * @link http://modext.github.com/extensions/smarty-renderer/index.html
...
class ESmartyViewRenderer extends CApplicationComponent implements IViewRenderer
{
	...
	/**
	 * @var null|string mod alias of the directory where your smarty plugins are located
	 ...
	 */
	public $smartyDir = 'system.exts.smarty';
	 ...
	 * will utilize mod autoload routine to load filters for compilation only
	 */
	...
	/**
	 * @var null|string mod alias of the directory where your smarty template-configs are located
	 */
	public $configDir;
	...
	public function init(){
		...
		Mod::import($this->smartyDir.'.*');

		// need this to avoid Smarty rely on spl autoload function,
		// this has to be done since we need the Mod autoload handler
		...
		// need this since Mod autoload handler raises an error if class is not found
		// Mod autoloader needs to be the last in the autoload chain
		spl_autoload_unregister('smartyAutoload');
		Mod::registerAutoloader('smartyAutoload');
		...
		$this->getSmarty()->setTemplateDir(Mod::app()->getViewPath());
		$compileDir = isset($this->config['compile_dir']) ?
					  $this->config['compile_dir'] : Mod::app()->getRuntimePath().'/smarty/compiled/';
		...
		//Register default template handler. This allow us to use mod aliases in the smarty templates.
		//You shoud set path without extension
		//for example {include file="application.views.layout.main"}
		$this->getSmarty()->default_template_handler_func = create_function('$type, $name', 'return Mod::getPathOfAlias($name) . "' . $this->fileExtension .'";'); 
 
		$this->getSmarty()->addPluginsDir(Mod::getPathOfAlias($this->smartyDir.'.plugins'));
		if(!empty($this->pluginsDir)){
		    $plugin_path = Mod::getPathOfAlias($this->pluginsDir);
		...
		if(!empty($this->configDir)){
			$this->getSmarty()->addConfigDir(Mod::getPathOfAlias($this->configDir));
		}
	}

	/**
	 * Add a pre or post filter defined in mod config
	 *
	 * @since 1.0.2
	 * @param string $type
	...
	public function registerFilter($type,$filter){
	    if (is_string($filter)){
		    if (!function_exists($filter)){
		        $filter_file = Mod::getPathOfAlias($this->pluginsDir).'/'.$type.'filter.'.$filter.'.php';
	...
	public function renderFile($context,$sourceFile,$data,$return)
	{
		...
		$data['this'] = $context;
		// Mod::app()->... is available as {Mod->...} (deprecated, use {Mod::app()->...} instead, Smarty3 supports this.)
		$data['Mod'] = Mod::app();
		// time and memory information
		$data['TIME'] = sprintf('%0.5f',Mod::getLogger()->getExecutionTime());
		$data['MEMORY'] = round(Mod::getLogger()->getMemoryUsage()/(1024*1024),2).' MB';

		// check if view file exists
		if(!is_file($sourceFile) || ($file=realpath($sourceFile))===false)
			throw new CException(Mod::t('modext','View file "{file}" does not exist.', array('{file}'=>$sourceFile)));
