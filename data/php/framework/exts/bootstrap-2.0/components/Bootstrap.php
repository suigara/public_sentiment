...
class Bootstrap extends CApplicationComponent
{
	...
	public function registerCoreCss()
	{
		...
		Mod::app()->clientScript->registerCssFile($this->getAssetsUrl().'/css/'.$filename);
	}
	...
	public function registerResponsiveCss()
	{
		...
		$filename = Mod_DEBUG ? 'bootstrap-responsive.css' : 'bootstrap-responsive.min.css';
		$cs->registerCssFile($this->getAssetsUrl().'/css/'.$filename);
	}

	/**
	 * Registers the Mod-specific CSS missing from Bootstrap.
	 * @since 0.9.11
	 */
	public function registerModCss()
	{
		Mod::app()->clientScript->registerCssFile($this->getAssetsUrl().'/css/Mod.css');
	}
	...
	{
		$this->registerJS(Mod::app()->clientScript->coreScriptPosition);
	...
	protected function registerPlugin($name, $selector = null, $options = array())
	{
		...
		if ($selector !== null)
		{
			...
			Mod::app()->clientScript->registerScript($key, "jQuery('{$selector}').{$name}({$options});");
		}
	}
	...
	*/
	protected function getAssetsUrl()
	{
		...
		else
		{
			...
			$assetsUrl = Mod::app()->assetManager->publish($assetsPath, true, -1, $this->forceCopyAssets);
