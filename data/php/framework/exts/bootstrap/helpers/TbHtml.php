...
class TbHtml extends CHtml
{
    ...
    protected static function addSpanClass($htmlOptions=array())
    {
        ...
        if (!empty($span)) {
            self::addCssClass('col-lg-' . $span, $htmlOptions);
        }
    }
    ...
    public static function abbr($text, $word = '', $htmlOptions = array())
    {
        ...
        $type = TbArray::popValue('type', $htmlOptions, false);
        if (!empty($type) && $type === TbHtml::TEXT_ABBR_INITIALISM) {
    ...
    public static function quote($text, $htmlOptions = array())
    {
        ...
        $small = TbArray::popValue('small', $htmlOptions);
        $smallOptions = TbArray::popValue('smallOptions', $htmlOptions, array());
...
EOD;
    ...
    public static function searchQueryField($name, $value = '', $htmlOptions = array())
    {
		...
		self::addCssClass('form-control', $htmlOptions);
        $control = self::textField($name, $value, $htmlOptions);
		return self::controls($control, array('row'=>true));
    }
    ...
    public static function activeControlGroup($type, $model, $attribute, $htmlOptions = array(), $data = array())
    {
        ...
        $label = TbArray::popValue('label', $htmlOptions);
        $labelOptions = TbArray::popValue('labelOptions', $htmlOptions, array());
        $layout = TbArray::popValue('formLayout', $htmlOptions);
        ...
        if (!empty($layout)) {
            if ($layout === TbHtml::FORM_LAYOUT_HORIZONTAL) {
                $controlClass = TbArray::popValue('class',$controlOptions,TbHtml::FORM_LAYOUT_HORIZONTAL_CONTROL_CLASS);
                self::addCssClass($controlClass, $controlOptions);
				$labelClass = TbArray::popValue('class',$labelOptions,TbHtml::FORM_LAYOUT_HORIZONTAL_LABEL_CLASS);
                self::addCssClass($labelClass.' control-label',$labelOptions);
            }
    ...
    protected static function activeTextInputField($type, $model, $attribute, $htmlOptions)
    {
        ...
        $placeHolder = TbArray::popValue('placeholder', $htmlOptions, false);
...
//        }
    ...
    public static function activeRadioButtonControlGroup($model, $attribute, $htmlOptions = array())
    {
        ...
        $header = $layout === TbHtml::FORM_LAYOUT_HORIZONTAL ? '<div class="form-group"><div class="col-lg-offset-2"><div class="radio"><label>' : '<div class="radio"><label>';
        $output = $header;
        $output .= $input;
        $output .= $model->getAttributeLabel($attribute);
        $footer = $layout === TbHtml::FORM_LAYOUT_HORIZONTAL ? '</label></div></div></div>' : '</label></div>';
        $output .= $footer;
        return $output;
    }
    ...
    public static function activeCheckBoxControlGroup($model, $attribute, $htmlOptions = array())
    {
        ...
        $input = isset($htmlOptions['input'])
            ? $htmlOptions['input']
            : self::createActiveInput($type, $model, $attribute, $htmlOptions);
        $header = $layout === TbHtml::FORM_LAYOUT_HORIZONTAL ? '<div class="form-group"><div class="col-lg-offset-2"><div class="checkbox"><label>' : '<div class="checkbox"><label>';
        $output = $header;
        $output .= $input;
        $output .= $model->getAttributeLabel($attribute);
        $footer = $layout === TbHtml::FORM_LAYOUT_HORIZONTAL ? '</label></div></div></div>' : '</label></div>';
    ...
    protected static function btn($type, $label, $htmlOptions = array())
    {
        ...
        $navbarbtn = TbArray::popValue('type', $htmlOptions, false);
        if($navbarbtn === TbHtml::BUTTON_TYPE_NAVBARBUTTON){
    ...
    public static function buttonGroup(array $buttons, $htmlOptions = array())
    {
        if (!empty($buttons)) {
            ...
            foreach ($buttons as $buttonOptions) {
                ...
                $own = TbArray::popValue('own', $buttonOptions, false);
                ...
                $type = TbArray::popValue('type', $buttonOptions, false);
                // todo: consider removing the support for htmlOptions.
    ...
    public static function nav($type, $items, $htmlOptions = array())
    {
        ...
        $justified = TbArray::popValue('justified', $htmlOptions, false);
        if ($justified) {
            ...
            foreach ($items as $itemOptions) {
                if (is_string($itemOptions)) {
					...
					if (empty($items)) {
                        ...
                        $itemOptions['linkOptions']['tabindex'] = -1;
						self::addCssClass('list-group-item', $itemOptions['linkOptions']);
                        $output .= self::menuLink($label, $url, $itemOptions);
                    } else {
                        $output .= self::menuDropdown($label, $url, $items, $itemOptions, $depth);
                    }
    ...
    public static function menuText($label, $htmlOptions = array())
    {
        ...
        $pull = TbArray::popValue('pull',$htmlOptions,false);
    ...
    {	
        ...
        foreach ($tabs as $tabOptions) {
            ...
            if (!empty($items)) {
                ...
                $id = $paneOptions['id'] = TbArray::popValue('id', $tabOptions, 'tab_' . $_counter .'_' . ++$i);
    ...
    public static function navbar($content, $htmlOptions = array())
    {
        ...
        $position = TbArray::popValue('position', $htmlOptions);
        if (!empty($position)) {
        ...
        TbArray::defaultValue('role','navigation',$htmlOptions);
    ...
    public static function navbarCollapseLink($target, $htmlOptions = array())
    {
        ...
        TbArray::defaultValue('type','button',$htmlOptions);
    ...
    public static function badge($label, $htmlOptions = array())
    {
        ...
        if (!empty($color)) {
            self::addCssClass('badge-' . $color, $htmlOptions);
			 ...
			 self::addCssClass('badge-default', $htmlOptions);
		}
        return self::tag('span', $htmlOptions, $label);
    }
