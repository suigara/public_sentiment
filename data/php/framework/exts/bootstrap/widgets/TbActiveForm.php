...
class TbActiveForm extends CActiveForm
{
    ...
    public function init()
    {
        ...
        $this->copyId();
        if ($this->stateful) {
            echo TbHtml::statefulFormBs($this->layout, $this->action, $this->method, $this->htmlOptions);
        } else {
            echo TbHtml::beginFormBs($this->layout, $this->action, $this->method, $this->htmlOptions);
        }
    }
    ...
    public function telField($model, $attribute, $htmlOptions = array())
    {
        ...
        return TbHtml::activeTelField($model, $attribute, $htmlOptions);
    }
   ...
   /**
     ...
     * @param array $htmlOptions additional HTML attributes.
     * @return string the generated row.
     * @see TbHtml::activeTelFieldControlGroup
     */
    public function telFieldControlGroup($model, $attribute, $htmlOptions = array())
    {
        $htmlOptions = $this->processRowOptions($model, $attribute, $htmlOptions);
        return TbHtml::activeTelFieldControlGroup($model, $attribute, $htmlOptions);
    }
    ...
    protected function processRowOptions($model, $attribute, $options)
    {
        ...
        $labelOptions = TbArray::popValue('labelOptions',$options,array());
    ...
    static private function setLabelOptionsByLayout($layout,$labelOptions=array()){
        ...
        if($layout === TbHtml::FORM_LAYOUT_INLINE){
            TbHtml::addCssClass('control-label',$labelOptions);
            TbHtml::addCssClass('sr-only',$labelOptions);
            return $labelOptions;
        }
        $labelClass = TbArray::popValue('class',$labelOptions,TbHtml::FORM_LAYOUT_HORIZONTAL_LABEL_CLASS);
        TbHtml::addCssClass('control-label',$labelOptions);
        TbHtml::addCssClass($labelClass,$labelOptions);
        return $labelOptions;
