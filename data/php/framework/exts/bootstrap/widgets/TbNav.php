...
class TbNav extends CWidget
{
    ...
    public function init()
    {
        ...
        $pull = TbArray::popValue('pull', $this->htmlOptions);

        if(!empty($pull)){
            if($pull === TbHtml::PULL_RIGHT||$pull === TbHtml::PULL_LEFT){
                TbHtml::addCssClass('pull-'.$pull,$this->htmlOptions);
            }
    ...
    {	
        if (isset($item['url']) && is_array($item['url']) ) {
			...
			if(!strcasecmp(trim($item['url'][0], '/'), $route)){
				...
				if($this->type == TbHtml::NAV_TYPE_NAVBAR ){
					$tmpar = explode("/",trim($item['url'][0], '/'));
					return strpos($route,$tmpar[0]) === 0;
				}			
			}            
        }
        return false;
